...
class HTTPSocksRelay(SocksRelay):
    ...
    def prepareRequest(self, data):
        ...
        # Locate exact boundary between headers and body
        headerSize = data.find(EOL + EOL)
        if headerSize != -1:
            raw_headers = data[:headerSize]
            initial_body = data[headerSize + 4:]
        else:
            raw_headers = data
            initial_body = b''

        raw_lines = raw_headers.split(EOL)
        if not raw_lines or not raw_lines[0]:
            return data

        # Sanitize absolute URI in request line (e.g. proxychains forwarding http://target/path)
        first_line = raw_lines[0]
        req_parts = first_line.split(b' ')
        if len(req_parts) >= 2:
            uri = req_parts[1]
            if uri.lower().startswith(b'http://') or uri.lower().startswith(b'https://'):
                scheme_end = uri.find(b'//')
                if scheme_end != -1:
                    path_start = uri.find(b'/', scheme_end + 2)
                    req_parts[1] = uri[path_start:] if path_start != -1 else b'/'
            first_line = b' '.join(req_parts)

        # === DEBUG: Log request line ===
        try:
            req_line_str = first_line.decode('utf-8', errors='replace')
            _dbg('>>> REQUEST: %s' % req_line_str)
        except Exception:
            pass
        # === END DEBUG ===
        ...
        for part in raw_lines[1:]:
            if part == b'':
                continue
            # Strip Basic/Negotiate auth headers
            if b'authorization' in part.lower():
                _dbg('>>> Stripped: %s' % part.decode('utf-8', errors='replace'))
                continue
            # Enforce persistent connection
            if b'connection: close' in part.lower():
                response.append(b'Connection: Keep-Alive')
                continue
            # Strip session cookie before forwarding
        ...
        # Ensure double CRLF termination between reconstructed headers and the body
        response.append(b'')
        senddata = EOL.join(response) + EOL + initial_body

        # Parse Content-Length case-insensitively to stream remaining packet buffers
        headers = self.getHeaders(data)
        content_length_val = None
        for k, v in headers.items():
            if str(k).lower() == 'content-length':
                content_length_val = v
                break

        if content_length_val is not None:
            try:
                bodySize = int(content_length_val)
                read_body_len = len(initial_body)
                while read_body_len < bodySize:
                    try:
                        chunk_to_read = min(self.packetSize, bodySize - read_body_len)
                        additional_data = self.socksSocket.recv(chunk_to_read)
                        if not additional_data:
                            LOG.debug('HTTP: Client closed connection while reading request body')
                            break
                        read_body_len += len(additional_data)
